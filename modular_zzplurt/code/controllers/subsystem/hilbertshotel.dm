...
/datum/controller/subsystem/hilbertshotel/proc/send_to_new_room(room_number, mob/user, template, obj/item/hilbertshotel/parentSphere)
	...
	if(lore_room_spawned && room_number == mysteryRoom)
		...
		else if(GLOB.supporter_list[user.ckey] < room_template.SUPPORTER_TIER)
			to_chat(user, span_warning("Tier [room_template.SUPPORTER_TIER] donator access required to use [room_template.name]."))
			return
